...
{
if (rightPaddle->Top > table -> Top-6) rightPaddle->Top -= 10;
}
//---------------------------------------------------------------------------

void __fastcall TForm1::rightPaddleDownTimer(TObject *Sender)
{
 if (rightPaddle->Top+rightPaddle->Height-10 < table -> Height) rightPaddle->Top += 10;
}
//---------------------------------------------------------------------------

void __fastcall TForm1::leftPaddleUpTimer(TObject *Sender)
{
 if (leftPaddle->Top > table -> Top-6) leftPaddle->Top -= 10;
}
//---------------------------------------------------------------------------

void __fastcall TForm1::leftPaddleDownTimer(TObject *Sender)
{
  if (leftPaddle->Top+leftPaddle->Height-10 < table -> Height) leftPaddle->Top += 10;
}
//---------------------------------------------------------------------------
...
if      (ball->Top-ball->Height/2 >= leftPaddle->Top+10  &&
        ball->Top-ball->Height/2 <= leftPaddle->Top+leftPaddle->Height-15 &&
        ball->Left <= leftPaddle->Left+leftPaddle->Width-15)
        {
        x=-x;
        }    //left bounce
        ...
        if(ball->Top-ball->Width/2 >= rightPaddle->Top+15 &&
        ball->Top-ball->Width/2 <= rightPaddle->Top+rightPaddle->Height-15 &&
        ball->Left+ball->Width >= rightPaddle->Left+15)
        {
        x=-x;
        }    //right bounce
